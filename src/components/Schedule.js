    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      {isLoading && (
        <div className="space-y-4">
          {[1, 2, 3].map((item) => <div key={item} className="h-32 animate-pulse rounded-2xl bg-gray-200 dark:bg-slate-800" />)}
        </div>
      )}
      ...
      {!isLoading && !error && selectedEvents.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-gray-200 bg-white px-6 py-16 text-center dark:border-slate-700 dark:bg-slate-800">
      ...
      {!isLoading && !error && selectedEvents.length > 0 && (
        <div className="space-y-4">
          {selectedEvents.map((event, index) => {
            ...
            return (
              <div key={`${event.start}-${event.courseCode}-${index}`} className="flex gap-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-3 flex items-center gap-2 border-t border-gray-100 pt-3 text-sm text-gray-500 dark:border-slate-700 dark:text-gray-400"><MapPin className="h-4 w-4" />{event.classRoom || 'N/A'}</p>
                </div>
              </div>
            );
          })}
