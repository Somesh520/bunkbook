  ...
  return (
    <div
      ...
      style={{
        ...
        '--cd-ink': ink,
        '--cd-size': `${size}px`,
        '--cd-figure': `${Math.round(size * 0.22)}px`
      }}
    >
      <svg
        ref={svgRef}
      ...
      <div className="pointer-events-none absolute inset-0 grid place-items-center" aria-hidden="true">
        <span className="inline-flex items-baseline gap-[0.06em] leading-none font-bold tracking-[-0.04em] tabular-nums [font-size:var(--cd-figure)] text-gray-900 dark:text-white">
          <span ref={figure} className="comet-dial__figure" />
          {unit ? <span className="text-[0.6em] opacity-80">{unit}</span> : null}
        </span>
      </div>
    </div>
  );
